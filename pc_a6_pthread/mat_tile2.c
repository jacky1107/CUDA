...
int *GetSubMatrix(int *matrix, int blockrow, int blockcol, int width);
int GetElement(int *matrix, int row, int col, int width);
void SetElement(int *matrix, int row, int col, int width, float value);
    ...
    for (int row = row_start; row < row_end; row++)
    {
        for (int col = column_start; col < column_end; col++)
        {
            for (int m = 0; m < (SIZE / TILE_WIDTH); ++m)
            {
                ...
                int *Md_sub = GetSubMatrix(A, blockrow, m, SIZE);
                // Get sub-matrix Nsub of N
                int *Nd_sub = GetSubMatrix(B, m, blockCol, SIZE);

                // Multiply Msub and Nsub together
                for (int k = 0; k < TILE_WIDTH; ++k)
                {
                    int Melement = GetElement(Md_sub, row, k, width);
                    int Nelement = GetElement(Nd_sub, k, col, width);
                    Pvalue += Melement * Nelement;
                }
            }
            SetElement(C, row, col, width, Pvalue);
        }
    }
    /*
    for (i = row_start; i < row_end; i++) { // hold row index of 'matrix1'
        for (j = column_start; j < column_end; j++) { // hold column index of 'matrix2'
